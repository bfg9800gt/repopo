...
def hark_request(username, txt): # запрос текста 
    ...
    text=''
    while len(text) < 3 or len(text) > 150:
        soup = BeautifulSoup(r['threads'][randint(1,6)]['posts'][randint(1,2)]['comment'], 'html.parser')
    ...
    # return f'{choice(eval(username))}, {choice(mezhdometie_list)}, {text}'
    return f'{choice(eval(username))}, {text}'
...
#                poslovica = f'{choice(poslovica_nachalo_list)} {poslovica_body}{choice(poslovica_konec_list)}'
                ...
                if username == 'PedrovichBot':
                    send_message(chat_id,username,choice(pedrovich_reaction))
